...
const assertType = (config, key, types) => {
    let pass;
    ...
    if (!pass) {
        throw new Error(`⚠️ The input variable '${key}' is of the wrong type.  Received '${typeof config[key]}', expected '${typesArray.join(" | ")}'.`);
    }
};
// NOTE: We're not using `getBooleanInput` from `actions/core` as ours isn't always a boolean…
const getForcedBooleanInput = (key) => {
    const value = (0,core.getInput)(key);
    if (value === undefined)
        return;
...
const getBooleanStringInput = (key) => {
    const value = getForcedBooleanInput(key);
    if (value !== null)
        return value;
    // NOTE: Sometimes this returns a non-boolean string.
    return (0,core.getInput)(key);
};
const getConfig = () => {
    const config = {
        package: (0,core.getInput)("package", { required: true }),
        directory: (0,core.getInput)("directory") || process.env.GITHUB_WORKSPACE,
        range: (0,core.getInput)("range", { required: true }),
        version_prerelease: getBooleanStringInput("version_prerelease") || false,
        allow_multiple_versions: getForcedBooleanInput("allow_multiple_versions") || false,
    };
...
function getTargetVersion(config = getConfig()) {
    ...
    if (dependencyVersion && devDependencyVersion) {
        throw new Error(`⚠️ Found '${config.package}' in both dependencies and devDependencies.`);
    }
    if (dependencyVersion)
        return dependencyVersion;
    if (devDependencyVersion)
        return devDependencyVersion;
    throw new Error(`⚠️ '${config.package}' was not found in dependencies or devDependencies.`);
}
function setTargetVersion(config = getConfig()) {
    const version = getTargetVersion(config);
    (0,core.setOutput)("target_version", version);
}
const foundRegex = /Found "([^"]+)"$/; // looking for `Found "package@version"`, matching on `package@version`
function getResolvedVersion(config = getConfig()) {
    const yarnWhyBuffer = (0,external_child_process_namespaceObject.spawnSync)("yarn", ["why", config.package], {
    ...
    const yarnWhyFound = yarnWhyLines.filter((line) => {
        ...
        return (line === null || line === void 0 ? void 0 : line.length) >= 0 && line.match(foundRegex);
    });
    const versionsList = yarnWhyFound.map((output) => {
        const found = output.match(foundRegex);
        if (!found[1])
            return; // this should be semver: `package@1.2.3-pre-release.0`
        const split = found[1].split("@"); // split so we can get the version.  NOTE: This may be `@org/package@version`—so there may be multiple '@' symbols.
        if (!split.length)
            return;
    ...
    if (versionsList.length > 1 && config.allow_multiple_versions !== true) {
        throw new Error(`⚠️ Found ${versionsList.length} versions for '${config.package}': [${versionsList.join(", ")}]`);
    }
    return versionsList[0];
}
function setResolvedVersion(config = getConfig()) {
    const version = getResolvedVersion(config);
    (0,core.setOutput)("resolved_version", version);
}

// EXTERNAL MODULE: ./node_modules/semver/index.js
var semver = __nccwpck_require__(1383);
var semver_default = /*#__PURE__*/__nccwpck_require__.n(semver);
;// CONCATENATED MODULE: ./src/enforce.ts



function enforceVersion(config = getConfig()) {
    const version = getResolvedVersion(config);
    const parsedVersion = semver_default().parse(version);
    if (!parsedVersion || !parsedVersion.version) {
        throw new Error(`⚠️ Could not parse a version out of '${version}'.`);
    }
    const range = config.range;
    const satisfies = semver_default().satisfies(parsedVersion.version, range, {
    ...
    if (config.version_prerelease === true) {
        ...
        if (parsedVersion.prerelease && parsedVersion.prerelease.length) {
            throw new Error(`⚠️ There should be no prerelease on '${parsedVersion.version}', received [${parsedVersion.prerelease.join(", ")}].`);
        }
    }
        ...
        if (tag !== parsedVersion.prerelease[0]) {
            throw new Error(`⚠️ Prerelease on '${parsedVersion.version}' does not match version_prerelease tag of '${tag}'.`);
        }
        if (identifier !== undefined &&
            String(identifier) !== String(parsedVersion.prerelease[1])) {
            throw new Error(`⚠️ Prerelease on '${parsedVersion.version}' does not match version_prerelease identifier of '${identifier}'.`);
        }
    }
}
